...
const Hero: React.FC<HeroProps> = ({ onLogoAnimationEnd }) => {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      ...
      setTimeout(() => {
        setShowLogo(false);
        onLogoAnimationEnd();
      }, 1000); // Wait for the logo animation to finish before hiding it
  ...
  const HeroContent = () => (
    <>
      ...
      />
      <header className="w-full absolute top-70 left-0 z-20 text-gray-800">
        <div className="px-4 py-6  items-center">
          <div className=" rounded-lg px-6 py-4  animate-fadeInUp flex justify-center flex-col items-center">
            <h1 className="text-3xl font-bold text-center xl:text-6xl  animate-fadeInUp delay-200">{t("hero.welcome")}</h1>
            <p className="text-lg text-center mt-2 font-semibold xl:text-2xl  animate-fadeInUp delay-400">{t("hero.subtitle")}</p>
          </div>
           
           <div className="mt-6 flex justify-center items-center animate-fadeInUp delay-500">
             <button
              className="bg-orion-gradient px-4 py-2 rounded hover:bg-[#8e77ad] transition-colors   w-1/2 animate-fadeInUp delay-600"
            >
              {t("hero.bookNow")}
            </button>
